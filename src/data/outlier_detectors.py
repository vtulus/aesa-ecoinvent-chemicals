...
def tukey_method(df_raw, colname: str):
    ...
    1º list - indices (as in `df_raw`) of PROBABLE outliers,
        i.e. those laying "far out" (outside the outer fence).
    2º list - indices (as in `df_raw`) of POSSIBLE outliers,
    ...
    df_raw : pandas.DataFrame
        Dataframe containing a colname of interest
    colname : str
    ...
    if colname not in df_raw.columns:
        raise ValueError(
            "Passed `colname` not in ´df_raw.columns´. Specify existing column label."
        )
    # quartile 1 (25%) # pylint: disable=invalid-name
    Q1 = df_raw[colname].quantile(0.25)
    # quartile 3 (75%) # pylint: disable=invalid-name
    Q3 = df_raw[colname].quantile(0.75)
    ...
    outliers_prob = []
    outliers_poss = []
    for index in df_raw[colname].index:
        check_value = df_raw[colname][index]  # value from df_raw[colname]
...
def tukey_method_bulk(df_raw, outlier_detection_fence="tight"):
    """Split `df_raw` in two dataframes: (1)-filtered data, (2)-detected outliers.
     ...
     based on Tukey's rule for EACH OF the columns of the `df_raw` and return 2 dataframes:
     1º dataframe - with the shape of `df_raw` contains only filtered data (no outliers)
     2º dataframe - with the shape of `df_raw` contains only detected outliers.
    ...
    df_treated : pandas.DataFrame
        Dataframe (with shape of `df_raw`) containing only filtered data,
        empty cells are `np.nan`
    df_outliers : pandas.DataFrame
        Dataframe (with shape of `df_raw`) containing only detected outliers,
        empty cells are `np.nan`
...
def mahalanobis_method(df_raw, alpha=0.001):  # pylint: disable=too-many-locals
    """Split `df_raw` in two dataframes: (1)-filtered data, (2)-detected outliers.
